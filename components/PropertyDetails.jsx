...
const PropertyDetails = ({ property }) => {
  // Check if property is null or undefined before accessing its properties
  if (!property) {
    return (
      <main>
  ...
  return (
    <main>
      ...
      <div className="mt-6 rounded-lg bg-white p-6 shadow-md">
        ...
        <ul className="space-y-02 grid list-none grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {property.amenities.map((amenity, index) => (
            <li key={index}>
              <FaCheck className="mr-2 inline text-green-600" /> {amenity}
            </li>
          ))}
        </ul>
      </div>
